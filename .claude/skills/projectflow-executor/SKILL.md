...
### Step 5.5: 简单项目需求文档生成（如适用）

**触发条件**: Phase 1 完成后，且 `{{COMPLEXITY}}` == simple

**说明**: 对于简单项目，不需要调用 brainstorming skill，直接生成需求文档模板

**参考**: 按照对应语言模板中 Step 5.5 的定义执行

- Python: 见 `python-complete-template.md` 中的 Step 5.5
- TypeScript: 见 `typescript-complete-template.md` 中的 Step 5.5
- Go: 见 `go-template.md` 中的 Step 5.5

**执行方式**: 使用 Write 工具，按模板写入需求文档

**输出位置**: `pjflow/{{VERSION_DIR}}/requirements.md`

**Skip if**: 复杂度为 medium 或 complex（这些项目通过 Phase 0 brainstorming 生成需求）
