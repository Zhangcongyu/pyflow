...
1. **用户明确指定优先（最高优先级）**
   - 用户明确参数（--simple, --medium, --complex）直接使用
   - 用户明确描述（"简单项目", "中等规模"）直接使用
   - 跳过自动检测，不进行覆盖
...
4. **始终展示判断依据并请求确认**
   - 展示检测到的三维参数
   - 说明判断依据（关键词匹配或技术特征）
   - 使用 AskUserQuestion 工具允许用户修正

5. **避免冲突时的猜测**
   - 检测冲突时（如"简单的平台"），优先采用更具体的描述
   - 不确定时询问，避免错误假设
...
**文档版本**: 2.0.0
**最后更新**: 2026-02-24
**更新内容**: 添加用户明确指定优先原则，更新检测流程图，增强交互确认机制
