...
**Agent 选择逻辑**：
根据项目类型选择 agent：
- FastAPI 项目 → python-development:fastapi-pro
- 其他 Python 项目 → python-development:python-pro
...
```
Task(
    subagent_type="python-development:fastapi-pro",  // ← FastAPI 时使用
    subject="实现功能使测试通过",
...
**调用**:
```
Skill(skill="python-development:async-python-patterns", args=f"""添加异步支持以提高性能
...
**调用**:
```
Skill(skill="python-development:python-performance-optimization", args=f"""分析并优化性能瓶颈
...
```
Task(
    subagent_type="python-development:python-performance-optimization",
    subject="深度性能优化",
